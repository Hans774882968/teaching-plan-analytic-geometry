...
import { parabolaDefinitionHQ } from './hardQuestionsConfig';
...
function Inner() {
  ...
  const checkAnswers = () => {
    const feedbacks = {};
    parabolaDefinitionHQ.quiz.forEach((_, index) => {
  ...
  return (
    <div className={styles.container}>
      <Section>
        ...
        <p>🔥 来测试一下你对抛物线的理解够不够深刻吧！</p>
        <Card>
          题目太难？回去学习：
          <Link to="/parabola-definition"><TPButton>抛物线基础知识讲解</TPButton></Link>
        </Card>
        {
          parabolaDefinitionHQ.quiz.map((quiz, index) => (
            <QuizContainer
              key={index}
              index={index}
              quiz={quiz}
...
export default function ParabolaHardQuestions() {
  return (
    <>
      <Helmet>
        <title>更难的抛物线习题</title>
      </Helmet>
      <Inner />
    </>
  );
